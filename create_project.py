...
BASE_DIR = "/mnt/d/icloud/iCloudDrive/developer"
PYTHON_ENV_VER = "3.10"
...
def create_project():
    ...
    run_command(["conda", "create", "--name", project_name, f"python={PYTHON_ENV_VER}", "-y"])
...
"""
    ...
    if is_gh_installed():
        run_command(["gh", "repo", "create", project_name, f"--{visibility}", "--source=.", "--remote=origin"])
        run_command(["git", "remote", "set-url", "origin", f"git@github.com:Zakk-Yang/{project_name}.git"])
        run_command(["git", "push", "-u", "origin", "main"])
        print(f"Git repository initialized and pushed to GitHub as a {visibility} repository")
    else:
        print("GitHub CLI (gh) is not installed. Please follow these steps to create a repository manually:")
